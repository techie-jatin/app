...
import { Send, Bell, Users, User, Info, AlertCircle, CheckCircle, Smartphone } from "lucide-react";
...
const TYPE_CFG: Record<NotifType, { icon: typeof Info; color: string; bg: string }> = {
  announcement: { icon: Bell,         color: "#3B82F6", bg: "rgba(59,130,246,0.1)" },
  reminder:     { icon: CheckCircle,  color: "#10B981", bg: "rgba(16,185,129,0.1)" },
  alert:        { icon: AlertCircle,  color: "#F59E0B", bg: "rgba(245,158,11,0.1)" },
  update:       { icon: Info,         color: "#8B5CF6", bg: "rgba(139,92,246,0.1)" },
};

const TEMPLATES = [
  { label: "Class Reminder", title: "Live Class Starting Soon", message: "Your live class is starting in 15 minutes. Join on time.", type: "reminder" as NotifType },
  { label: "Assignment Due", title: "Assignment Deadline Tomorrow", message: "Please submit your assignment before the deadline.", type: "alert" as NotifType },
  { label: "Quiz Alert", title: "Quiz Goes Live Today", message: "Your scheduled quiz is now active. Complete before time limit.", type: "alert" as NotifType },
  { label: "Result Published", title: "Results Are Out!", message: "Your quiz/assignment results have been published. Check your score.", type: "update" as NotifType },
];
...
export function AdminNotificationCenter() {
  ...
  const applyTemplate = (t: typeof TEMPLATES[0]) => setForm(p => ({ ...p, title: t.title, message: t.message, type: t.type }));
    ...
    <div className="flex h-screen" style={{ background: BG }}>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="flex items-center gap-2 px-6 py-4 flex-shrink-0" style={{ background: CARD, borderBottom: `1px solid ${BORDER}` }}>
          ...
          <div>
            <h1 className="text-lg font-bold" style={{ color: TEXT }}>Notification Center</h1>
          ...
          <div className="grid lg:grid-cols-5 gap-6">
            <div className="lg:col-span-2 space-y-4">
              <div className="rounded-xl p-5 space-y-4" style={{ background: CARD, border: `1px solid ${BORDER}` }}>
                <h2 className="text-sm font-semibold" style={{ color: TEXT }}>Send Notification</h2>

                <div>
                  <p className="text-xs mb-2" style={{ color: MUTED }}>Quick Templates</p>
                  <div className="grid grid-cols-2 gap-2">
                    {TEMPLATES.map(t => {
                      const cfg = TYPE_CFG[t.type];
                      return (
                        <button key={t.label} onClick={() => applyTemplate(t)} className="text-left px-2.5 py-2 rounded-lg text-xs" style={{ background: cfg.bg, color: cfg.color, border: `1px solid ${cfg.color}30` }}>
                          {t.label}
                        </button>
                      );
                    })}
                ...
                <div>
                  ...
                  <textarea value={form.message} onChange={e => setForm(p => ({ ...p, message: e.target.value }))} rows={3} placeholder="Notification body…"
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {(Object.entries(TYPE_CFG) as [NotifType, typeof TYPE_CFG[NotifType]][]).map(([key, cfg]) => (
                      <button key={key} onClick={() => setForm(p => ({ ...p, type: key }))}
                        className="flex items-center gap-2 px-2.5 py-2 rounded-lg text-xs capitalize" style={{ background: form.type === key ? cfg.bg : SURFACE, color: form.type === key ? cfg.color : MUTED, border: `1px solid ${form.type === key ? `${cfg.color}40` : BORDER}` }}>
                        <cfg.icon className="w-3.5 h-3.5" /> {key}
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  <label className="block text-xs mb-1" style={{ color: MUTED }}>Target</label>
                  ...
                  <select value={form.batchId} onChange={e => setForm(p => ({ ...p, batchId: e.target.value }))}
                    ...
                    <option value="">Or select a batch…</option>
            ...
            <div className="lg:col-span-3">
              <div className="rounded-xl overflow-hidden" style={{ background: CARD, border: `1px solid ${BORDER}` }}>
                <div className="px-5 py-4" style={{ borderBottom: `1px solid ${BORDER}` }}>
                  <h2 className="text-sm font-semibold" style={{ color: TEXT }}>Sent History ({notifications.length})</h2>
                </div>
                <div className="divide-y overflow-y-auto" style={{ borderColor: BORDER, maxHeight: "calc(100vh - 220px)" }}>
                      ...
                      <div key={n.id} className="px-5 py-4 flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-2 mb-0.5">
                            ...
                            <span className="text-[10px]" style={{ color: MUTED }}>{new Date(n.sentAt).toLocaleDateString("en-IN", { day: "2-digit", month: "short" })}</span>
                          </div>
                          <p className="text-xs mb-1.5 leading-relaxed" style={{ color: MUTED }}>{n.message}</p>
                          <div className="flex items-center gap-2">
                            <Smartphone className="w-3 h-3" style={{ color: MUTED }} />
                            <span className="text-[10px]" style={{ color: MUTED }}>To: {batch ? batch.name : n.targetBatch}</span>
                          </div>
                        </div>
                      </div>
                    );
                  ...
                  {!sorted.length && (
                    <div className="py-12 text-center">
                      ...
                      <p className="text-sm" style={{ color: MUTED }}>No notifications sent yet</p>
                    </div>
                  )}
                </div>
              </div>
