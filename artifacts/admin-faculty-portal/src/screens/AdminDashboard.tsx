...
import { Activity, AlertTriangle, Award, BarChart2, Bell, BookOpen, Calendar, CheckCircle, ChevronRight, Clock, FileText, GraduationCap, Layers, Menu, PlayCircle, Settings, TrendingUp, UserCheck, Users, X } from "lucide-react";
...
const navItems = [
  { icon: BarChart2, label: "Dashboard", active: true, path: "/admin/dashboard" },
    ...
    <div className="flex h-screen overflow-hidden font-['Poppins'] w-full" style={{ background: BG, color: TEXT }}>
      ...
      <aside className="hidden md:flex w-60 flex flex-col flex-shrink-0" style={{ background: CARD, borderRight: `1px solid ${BORDER}` }}>
        ...
        <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
          {navItems.map((item) => (
            <button key={item.label} onClick={() => item.path && navigate(item.path)} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all" style={item.active ? { background: "rgba(37,99,235,0.15)", color: "#3B82F6", fontWeight: 600 } : { color: MUTED }}>
