...
const upcoming = [
  { title: "Live: Options Chain Deep Dive", time: "Today, 6:00 PM", live: true, path: "/live/waiting" },
  { title: "Quiz 4 — Risk Management", time: "Due: Jun 26", live: false, path: "/quiz" },
  { title: "Assignment 2 — Chart Analysis", time: "Due: Jun 28", live: false, path: "/assignment" },
];

const navItems = [
  { icon: TrendingUp, label: "Home", path: "/home", active: true },
  { icon: BookOpen, label: "Courses", path: "/course", active: false },
  { icon: Calendar, label: "Schedule", path: "/schedule", active: false },
  { icon: BarChart2, label: "Progress", path: "/progress", active: false },
  { icon: Award, label: "Profile", path: "/profile", active: false },
];
    ...
    <div className="w-[390px] h-[844px] flex flex-col overflow-hidden font-['Poppins']" style={{ background: BG, color: TEXT }}>
      ...
      <div className="px-5 pt-4 pb-5 flex-shrink-0" style={{ background: NAVY }}>
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="relative" onClick={() => navigate("/notifications")} style={{ cursor: "pointer" }}>
      ...
      <div className="flex-1 overflow-y-auto px-4 pt-4 space-y-4 pb-20">
        <div className="rounded-2xl p-4 flex items-start gap-3" style={{ background: RED }}>
          ...
          <button
            onClick={() => navigate("/live/waiting")}
            className="bg-white text-xs font-bold px-3 py-1.5 rounded-xl flex-shrink-0"
            style={{ color: RED }}>
            Join
          </button>
        </div>
        ...
        <div>
          ...
          <div className="space-y-2.5">
            {lectures.map((l, i) => (
              <div
                key={i}
                onClick={() => !l.locked && navigate("/course/player")}
                className="flex items-center gap-3 rounded-2xl p-3"
                style={{ background: CARD, border: `1px solid ${BORDER}`, opacity: l.locked ? 0.55 : 1, boxShadow: "0 1px 3px rgba(0,0,0,0.04)", cursor: l.locked ? "not-allowed" : "pointer" }}>
